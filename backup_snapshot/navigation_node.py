...
class NavigationNode(Node):
    def __init__(self):
        ...
        self.goal_heading_weight = min(
            1.0, max(0.0, float(p('goal_heading_weight'))))
    ...
    def _declare_parameters(self):
        defaults = {
            ...
            # Bias steering toward the target bearing immediately, so a
            # diagonal goal produces an arc from the first control cycle.
            'goal_heading_gain': 1.00,
            'goal_heading_weight': 0.80,
        }
    ...
    def _convert_to_steering(command, gain, max_angular, min_speed_factor,
        ...
        # Preserve the requested planar speed even when the policy's forward
        # component is near zero.  A diagonal/side-biased goal must produce a
        # forward arc; forcing x to zero makes gear-6 vehicles spin in place.
    ...
    @staticmethod
    def _apply_goal_heading(command, heading_error, gain, weight,
                            max_angular, min_speed_factor):
        guided = np.asarray(command, dtype=np.float32).copy()
        if guided.shape != (3,) or not np.isfinite(guided).all() or \
                not math.isfinite(heading_error):
            return np.zeros(3, dtype=np.float32)
        goal_angular = float(np.clip(
            gain * heading_error, -max_angular, max_angular))
        guided[2] = np.clip(
            (1.0 - weight) * guided[2] + weight * goal_angular,
            -max_angular, max_angular)
        heading_speed = max(
            min_speed_factor, max(0.0, math.cos(abs(heading_error))))
        guided[0] *= heading_speed
        guided[1] = 0.0
        return guided
    ...
    def _run_policy(self, depth):
        ...
        # Keep the filter history in policy-command space. Storing the
        # converted steering command here mixes yaw into the next policy frame.
        ...
        if self.steering_drive_mode:
            ...
            goal_bearing = math.atan2(
                float(self.goal[1] - robot_position[1]),
                float(self.goal[0] - robot_position[0]))
            heading_error = math.atan2(
                math.sin(goal_bearing - yaw), math.cos(goal_bearing - yaw))
            filtered = self._apply_goal_heading(
                filtered, heading_error, self.goal_heading_gain,
                self.goal_heading_weight, self.steering_max_angular,
                self.steering_min_speed_factor)
        output = Twist()
