...
class NavigationNode(Node):
    def __init__(self):
        ...
        self.goal_heading_gain = max(0.0, float(p('goal_heading_gain')))
        self.heading_align_angle = math.radians(max(
            0.0, float(p('heading_align_angle_deg'))))
        self.heading_align_speed_factor = min(
            1.0, max(0.0, float(p('heading_align_speed_factor'))))
    ...
    def _declare_parameters(self):
        defaults = {
            ...
            'goal_heading_gain': 1.20,
            'heading_align_angle_deg': 15.0,
            'heading_align_speed_factor': 0.25,
        }
    ...
    def _convert_to_steering(command, gain, max_angular, min_speed_factor,
                             model_yaw_weight, goal_heading=0.0,
                             goal_heading_gain=1.0,
                             heading_align_angle=math.radians(15.0),
                             heading_align_speed_factor=0.25):
        ...
        policy_heading = math.atan2(lateral, max(abs(forward), 1e-4))
        goal_heading = math.atan2(math.sin(goal_heading), math.cos(goal_heading))
        if abs(goal_heading) > heading_align_angle:
            speed_factor = heading_align_speed_factor
        else:
            speed_factor = max(min_speed_factor, max(0.0, math.cos(goal_heading)))
        # A lateral-dominant policy action still needs forward motion in drive
        # gear; otherwise steering mode can only turn in place.
        converted[0] = direction * planar_speed * speed_factor
        converted[1] = 0.0
        goal_turn = goal_heading_gain * goal_heading
        policy_turn = model_yaw_weight * (
            float(converted[2]) + direction * gain * policy_heading)
        if abs(goal_turn) > 1e-4:
            # The waypoint bearing is authoritative. Limit the learned
            # residual so it can refine a turn but cannot cancel or reverse it.
            residual_limit = 0.5 * abs(goal_turn)
            policy_turn = float(np.clip(
                policy_turn, -residual_limit, residual_limit))
        converted[2] = np.clip(goal_turn + policy_turn, -max_angular, max_angular)
        return converted

    @staticmethod
    def _goal_heading_in_body(goal, robot_position, rotation):
        delta_world = np.asarray(goal, dtype=np.float32) - np.asarray(
            robot_position, dtype=np.float32)
        delta_body = rotation.inv().apply(delta_world)
        return math.atan2(float(delta_body[1]), float(delta_body[0]))
    ...
    def _run_policy(self, depth):
        ...
        goal_heading = self._goal_heading_in_body(
            self.goal, robot_position, rotation)
        ...
        if not np.isfinite(command).all():
            ...
            self._publish_stop()
            return
        filtered = (np.array([0.9, 0.5, 0.5]) * command +
                    np.array([0.1, 0.5, 0.5]) * self.previous_cmd)
        # Keep filter history in policy space. Steering conversion clears the
        # lateral component, so storing converted commands would erase the
        # model's lateral intent on the next cycle.
        self.previous_cmd = filtered.copy()
        if self.steering_drive_mode:
            filtered = self._convert_to_steering(
                ...
                self.steering_model_yaw_weight, goal_heading,
                self.goal_heading_gain, self.heading_align_angle,
                self.heading_align_speed_factor)
            self.get_logger().info(
                'Route steering: heading_error=%.3f, linear_x=%.3f, angular_z=%.3f' %
                (goal_heading, filtered[0], filtered[2]),
