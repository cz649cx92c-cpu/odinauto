...
class PointCloudSafety(Node):
    def __init__(self):
        ...
        self.nearest_front_distance = math.inf
    ...
    def _declare_parameters(self):
        defaults = {
            ...
            'front_min_distance': 0.05,
            # Turn early enough to follow an arc instead of changing to the
            # chassis lateral gear after the obstacle is already very close.
            'front_max_distance': 0.90,
            ...
            # Near-obstacle fallback stays in drive gear and follows a low-speed
            # arc. Normal, farther obstacle avoidance remains the SRU policy's job.
            'enable_turn_avoidance': True,
            'avoidance_turn_speed': 0.06,
            'avoidance_turn_angular': 0.30,
            'avoidance_stop_distance': 0.28,
            'avoidance_full_speed_distance': 0.60,
            ...
            'avoidance_turn_timeout': 8.0,
            'turn_before_lateral_time': 2.5,
            'lateral_fallback_distance': 0.32,
            'mode_switch_pause': 0.20,
            'lateral_escape_speed': 0.06,
            'lateral_escape_min_distance': 0.18,
            'lateral_escape_max_distance': 0.45,
            'lateral_escape_timeout': 6.0,
    ...
    def _process_latest_cloud(self):
        ...
        self.nearest_front_distance = nearest_distance
        self.last_cloud_time = receive_time
        self.obstacle_pub.publish(Bool(data=self.block_forward))
    ...
    def _should_use_lateral(self, now):
        p = lambda name: self.get_parameter(name).value
        turn_elapsed = (self.episode_start_time is not None and
                        now - self.episode_start_time >=
                        float(p('turn_before_lateral_time')))
        return (self.nearest_front_distance <= float(p('lateral_fallback_distance')) or
                turn_elapsed)

    def _begin_lateral_escape(self, now):
        pose = self.latest_odom.pose.pose
        quaternion = pose.orientation
        yaw = math.atan2(
            2.0 * (quaternion.w * quaternion.z + quaternion.x * quaternion.y),
            1.0 - 2.0 * (quaternion.y ** 2 + quaternion.z ** 2))
        self.lateral_start_pose = np.array([pose.position.x, pose.position.y])
        self.lateral_axis = np.array([-math.sin(yaw), math.cos(yaw)])
        self._enter_state('LATERAL_ESCAPE', now)

    def _lateral_escape_distance(self):
        if self.lateral_start_pose is None or self.latest_odom is None:
            return 0.0
        pose = self.latest_odom.pose.pose.position
        position = np.array([pose.x, pose.y])
        return self.avoidance_direction * float(np.dot(
            position - self.lateral_start_pose, self.lateral_axis))
    ...
    def _control_tick(self):
        ...
        if state in ('IDLE', 'CRUISE'):
            if self.block_forward and bool(p('enable_turn_avoidance')):
                ...
                if direction == 0:
                    ...
                    self._enter_state('AVOID_TURN', now)
            else:
                self._enter_state('CRUISE', now)
            ...
            if self._side_blocked(self.avoidance_direction):
                self._enter_state('WAIT_BLOCKED', now)
            elif now - self.state_enter_time >= float(p('avoidance_turn_timeout')):
                self._enter_state('ABORT_STOP', now)
            elif self.front_clear_frames >= int(p('front_clear_frames_required')):
                self._enter_state('RECOVER_FORWARD', now)
            elif self._should_use_lateral(now):
                self._enter_state('PAUSE_TO_LATERAL', now)
            else:
                stop_distance = float(p('avoidance_stop_distance'))
                full_speed_distance = max(
                    stop_distance + 0.01, float(p('avoidance_full_speed_distance')))
                clearance_factor = min(1.0, max(
                    0.0, (self.nearest_front_distance - stop_distance) /
                    (full_speed_distance - stop_distance)))
                output.linear.x = float(p('avoidance_turn_speed')) * clearance_factor
                output.linear.y = 0.0
                output.angular.z = (self.avoidance_direction *
                                    float(p('avoidance_turn_angular')))

        elif state == 'PAUSE_TO_LATERAL':
            if self._side_blocked(self.avoidance_direction):
                self._enter_state('WAIT_BLOCKED', now)
            elif self.front_clear_frames >= int(p('front_clear_frames_required')):
                self._enter_state('PAUSE_TO_DRIVE', now)
            elif now - self.state_enter_time >= float(p('mode_switch_pause')):
                self._begin_lateral_escape(now)

        elif state == 'LATERAL_ESCAPE':
            distance = self._lateral_escape_distance()
            if self._side_blocked(self.avoidance_direction):
                self._enter_state('WAIT_BLOCKED', now)
            elif (distance >= float(p('lateral_escape_max_distance')) or
                  now - self.state_enter_time >= float(p('lateral_escape_timeout'))):
                self._enter_state('ABORT_STOP', now)
            elif (distance >= float(p('lateral_escape_min_distance')) and
                  self.front_clear_frames >= int(p('front_clear_frames_required'))):
                self._enter_state('PAUSE_TO_DRIVE', now)
            else:
                output.linear.y = (self.avoidance_direction *
                                   float(p('lateral_escape_speed')))
            ...
            if (not self.block_forward and
                ...
                if direction:
                    ...
                    next_state = ('PAUSE_TO_LATERAL' if self._should_use_lateral(now)
                                  else 'AVOID_TURN')
                    self._enter_state(next_state, now)
