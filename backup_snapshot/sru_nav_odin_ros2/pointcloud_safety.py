...
class PointCloudSafety(Node):
    ...
    def _declare_parameters(self):
        defaults = {
            ...
            'front_max_distance': 3.00,
            'front_half_width': 0.35,
            'side_min_distance': 0.20,
            'side_max_distance': 1.00,
            'side_half_length': 0.45,
    ...
    def _process_latest_cloud(self):
        ...
        if fit_due:
            ...
            if plane is not None:
                ...
                # Never continue with a cached plane after a failed scheduled
                # validation; changed terrain must fail closed.
                self.cached_ground_plane = None
        ...
        minimum = max(1, math.ceil(int(p('minimum_obstacle_points')) / sample_stride))
    ...
    def _assisted_lateral_velocity(self, requested_y, now):
        p = lambda name: self.get_parameter(name).value
        threshold = float(p('lateral_command_threshold'))
        requested_direction = 1 if requested_y >= threshold else (
            -1 if requested_y <= -threshold else 0)
        requested_side_safe = ((requested_direction > 0 and not self.block_left) or
                               (requested_direction < 0 and not self.block_right))
        if requested_side_safe:
            self.avoidance_direction = requested_direction
            self.avoidance_direction_since = now
            return requested_y

        hold_time = float(p('avoidance_direction_hold_time'))
        hold_previous = (self.avoidance_direction_since is not None and
                         now - self.avoidance_direction_since < hold_time)
        direction = self._select_lateral_direction(
            self.block_left, self.block_right,
            self.left_obstacle_count, self.right_obstacle_count,
            self.avoidance_direction, hold_previous,
            int(p('avoidance_clear_margin_points')))
        if direction == 0:
            self.avoidance_direction = 0
            self.avoidance_direction_since = None
            return 0.0
        if direction != self.avoidance_direction:
            ...
            self.avoidance_direction_since = now
        return direction * float(p('lateral_avoidance_speed'))

    def _command_callback(self, message):
        output = Twist()
        now = time.monotonic()
        cloud_stale = self.last_cloud_time is None or \
            now - self.last_cloud_time > self.cloud_timeout
        if ((cloud_stale or not self.safety_valid) and self.fail_safe) or \
                (self.block_forward and self.block_left and self.block_right):
            self.avoidance_direction = 0
            self.avoidance_direction_since = None
        ...
        forward_threshold = float(self.get_parameter('forward_command_threshold').value)
        forward_requested = output.linear.x > forward_threshold
        if self.block_forward and output.linear.x > 0.0:
            output.linear.x = 0.0
            if forward_requested and bool(
                    self.get_parameter('enable_lateral_avoidance_assist').value):
                output.linear.y = self._assisted_lateral_velocity(output.linear.y, now)
        if not forward_requested or not self.block_forward:
            self.avoidance_direction = 0
            self.avoidance_direction_since = None
        if self.block_left and output.linear.y > 0.0:
            output.linear.y = 0.0
        if self.block_right and output.linear.y < 0.0:
            output.linear.y = 0.0
        self.output_pub.publish(output)
